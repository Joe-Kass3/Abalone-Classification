...
#including likelihood was less effective than feature removal w/ booleans for sex
def calc_likelihood_sex(train_data):
    ll_sex = {
        "M": [0,0,0,0],
        "F": [0,0,0,0],
        "I": [0,0,0,0]}
    ...
    for i in range(len(train_data)):
        for j in range(1,4):
            if(train_data['Sex'] == "M" and train_data['Category Age'] == j): 
                ll_sex["M"][j] += 1
            elif(train_data['Sex'] == "F" and train_data['Category Age'] == j): 
                ll_sex["F"][j] += 1
            elif(train_data['Sex'] == "I" and train_data['Category Age'] == j): 
                ll_sex["I"][j] += 1

    age_counts = train_data['Category Age'].value_counts(ascending=True)

    array = [0,age_counts[1],age_counts[2],age_counts[3]]
    for i in range(1,4):
        ll_sex["M"][i] = round((ll_sex["M"][i]/array[i]), 3)
        ll_sex["F"][i] = round((ll_sex["F"][i]/array[i]), 3)
        ll_sex["I"][i] = round((ll_sex["I"][i]/array[i]), 3)
...
#### this is is most likely not what he wanted in terms of selecting features ##
## this just iteratres through literally every possible feature combo ##
...
## other option ##
# feature_names = ['Sex','Length','Diameter','Height','Whole weight','Shucked weight','Viscera weight','Shell weight']
feature_names = ['Sex_F', 'Sex_I', 'Sex_M','Length','Diameter','Height','Whole weight','Shucked weight','Viscera weight','Shell weight']
...
working_features = ['Sex_F', 'Sex_I', 'Sex_M', 'Length','Diameter','Height','Whole weight','Shucked weight','Viscera weight','Shell weight']
removed_features = []
attempted_combos = {}
