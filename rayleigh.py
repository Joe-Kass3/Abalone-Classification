...
def ray_calculate_probability(num, mean, std):
	"""
...
def ray_calc_class_probabilities(data, summaries, row):
	"""
	...
	for class_values, summaries in summaries.items():
		for i in range(len(summaries)):
			...
			probabilities[class_values] = prior_prob[class_values] * ray_calculate_probability(row[i+1], mean, std)
	
	return probabilities


def ray_prediction(train_data, model, row):
	"""
	Takes in training data and uses that to calculate the probability for each row in the new test data
	"""
	probabilities = ray_calc_class_probabilities(train_data, model, row)
...
# print(calc_class_probabilities(train_data, model, row))
...
for i in range(len(test_data)):
	...
	label = ray_prediction(train_data, model, test_data.iloc[i, :])
	print("\n~~~\n")
